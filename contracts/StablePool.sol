...
contract StablePool is IStablePool, Ownable {
  ...
  constructor(address _owner, address _litAgent, address _token, address[] memory _initialTokens) Ownable(_owner) {
    LIT_AGENT = _litAgent;
    TOKEN = token;
  ...
  function withdraw(address _preferredToken, uint256 _amount) external {
    require(tokenTotals[_preferredToken] >= _amount, InsufficientTokenBalance(_preferredToken));
  ...
  function getBalance(address user, address _token) external view returns (uint256) {
    return IERC20(TOKEN).balanceOf(user);
  }

  function depositYield(address _token, uint256 _amount) external {
    _deposit(_token, _amount);
    emit YieldDeposited(msg.sender, _amount);
    EcoDollar(TOKEN).rebase(_amount);
  }
}
