...
import {CountersUpgradeable} from '@openzeppelin/contracts-upgradeable/utils/CountersUpgradeable.sol'; // look into this
import {PausableUpgradeable} from '@openzeppelin/contracts-upgradeable/security/PausableUpgradeable.sol'; // look into this
import {ECDSAUpgradeable} from '@openzeppelin/contracts-upgradeable/utils/cryptography/ECDSAUpgradeable.sol'; // look into this
import {EIP712Upgradeable} from '@openzeppelin/contracts-upgradeable/utils/cryptography/EIP712Upgradeable.sol';
import {AccessControlUpgradeable} from '@openzeppelin/contracts-upgradeable/access/AccessControlUpgradeable.sol';
import {IERC20MetadataUpgradeable} from '@openzeppelin/contracts-upgradeable/token/ERC20/extensions/IERC20MetadataUpgradeable.sol'; // look into this
import {IERC20PermitUpgradeable} from '@openzeppelin/contracts-upgradeable/token/ERC20/extensions/IERC20PermitUpgradeable.sol'; // look into this

/**
 * @title Eco Protocol USD Contract
 * @notice Based off Mountain protocol's USDM contract
 */
contract RebaseTokenExample is
  ...
  EIP712Upgradeable
{
  ...
  // ECO Errors
  error ECOInvalidMintReceiver(address receiver);
  error ECOInvalidBurnSender(address sender);
  error ECOInsufficientBurnBalance(address sender, uint256 shares, uint256 sharesNeeded);
  error ECOInvalidRewardMultiplier(uint256 rewardMultiplier);
  error ECOBlockedSender(address sender);
  error ECOInvalidBlockedAccount(address account);
  error ECOPausedTransfers();
  ...
  function _mint(address to, uint256 amount) private {
    require(to != address(0), ECOInvaliMintReceiver(to));
  ...
  function _burn(address account, uint256 amount) private {
    if (account == address(0)) {
      revert ECOInvalidBurnSender(account);
    }
    ...
    if (accountShares < shares) {
      revert ECOInsufficientBurnBalance(account, accountShares, shares);
    }
  ...
  function _beforeTokenTransfer(address from, address /* to */, uint256 /* amount */) private view {
    ...
    if (isBlocked(from)) {
      revert ECOBlockedSender(from);
    }
    // Useful for scenarios such as preventing trades until the end of an evaluation
    // period, or having an emergency switch for freezing all token transfers in the
    // event of a large bug.
    if (paused()) {
      revert ECOPausedTransfers();
    }
  }
  ...
  function _blockAccount(address account) private {
    if (isBlocked(account)) {
      revert ECOInvalidBlockedAccount(account);
    }
  ...
  function rebase(uint256 _amount) public onlyRole(oracle) {
    _setRewardMultiplier((totalSupply() + _amount) * _BASE / totalShares());
  }
